---
summary: "`moltbot plugins` 的 CLI 参考（列表，安装，启用/禁用，诊断）"
read_when:
  - 您想要安装或管理进程内网关插件
  - 您想要调试插件加载失败
---
...
捆绑插件随 Moltbot 一起发布但默认禁用。使用 `plugins enable` 
激活它们。

所有插件必须附带一个 `moltbot.plugin.json` 文件，其中包含内联 JSON 模式
（`configSchema`，即使为空）。缺失/无效的清单或模式会阻止
插件加载并导致配置验证失败。
