...
TypeBox 是一个以 TypeScript 为主的架构库。我们使用它来定义 **网关
WebSocket 协议**（握手、请求/响应、服务器事件）。这些架构
驱动 **运行时验证**、**JSON 架构导出** 和 **Swift 代码生成**
macOS 应用。单一真相来源；其他一切都是生成的。

如果您想要更高级别的协议上下文，请从
[网关架构](/concepts/architecture) 开始。
...
第一个帧 **必须** 是 `connect` 请求。此后，客户端可以调用
...
- 生成的 JSON 架构：`dist/protocol.schema.json`
- 生成的 Swift 模型：`apps/macos/Sources/MoltbotProtocol/GatewayModels.swift`

## 当前流水线

- `pnpm protocol:gen`
  - 将 JSON 架构（draft‑07）写入 `dist/protocol.schema.json`
...
然后将 `"system.echo"` 添加到 `src/gateway/server.ts` 中的 `METHODS`。
...
- `GatewayFrame` 枚举，带有 `req`、`res`、`event` 和 `unknown` 情况
- 强类型有效载荷结构/枚举
- `ErrorCode` 值和 `GATEWAY_PROTOCOL_VERSION`

未知帧类型作为原始有效载荷保留以实现向前兼容。
...
- 大多数对象使用 `additionalProperties: false` 以获得严格有效载荷。
- `NonEmptyString` 是 ID 和方法/事件名称的默认值。
- 顶层 `GatewayFrame` 在 `type` 上使用 **鉴别器**。
- 有副作用的方法通常需要在参数中包含 `idempotencyKey`
  （示例：`send`、`poll`、`agent`、`chat.send`）。

## 活架构 JSON

生成的 JSON 架构在仓库中位于 `dist/protocol.schema.json`。发布的
原始文件通常可在以下位置获得：
