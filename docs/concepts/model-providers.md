...
此页面涵盖 **LLM/模型提供商**（不是像 WhatsApp/Telegram 这样的聊天通道）。
...
Moltbot 随附 pi‑ai 目录。这些提供商需要 **无**
`models.providers` 配置；只需设置认证 + 选择模型。
...
Kimi Code 使用专用端点和密钥（与 Moonshot 分离）：
...
参见 [/providers/qwen](/providers/qwen) 获取设置详细信息和说明。
...
参见 [/providers/minimax](/providers/minimax) 获取设置详细信息、模型选项和配置片段。
...
Ollama 在本地运行于 `http://127.0.0.1:11434/v1` 时自动检测。参见 [/providers/ollama](/providers/ollama) 获取模型推荐和自定义配置。
...
另见：[/gateway/configuration](/gateway/configuration) 获取完整配置示例。